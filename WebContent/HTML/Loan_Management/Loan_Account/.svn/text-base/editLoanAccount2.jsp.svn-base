...
<head>
	...
	<title>Edit Loan Account</title>
 	...
 	<script type="text/javascript" src="/NSEP_System/HTML/Loan_Management/Loan_Account/editLoanAccount2.js">
	</script>
	<script type="text/javascript" src="/NSEP_System/JavaScript/CalendarControl.js">
	</script>
</head>
<%
...
<% 	LoanAccountBean laObj = new LoanAccountBean();
	laObj=(LoanAccountBean)request.getAttribute("editObj");
	
	String date = laObj.getStartDate();
    ...
    <div id="site_content">
      ...
      <div id="content">
		<h1>Edit Loan Account </h1>			
			...
			<tr>
				<td>Start Date</td>
				<td><input type="textbox" readonly="readonly" "start_date" name="start_date" size="30" value="<%=month+"-"+day+"-"+year %>" onfocus="showCalendarControl(this)" onblur="blurColor(this)"><label id="start_date_label"></label></td>
			</tr>			
			<tr>
				<td>Bank</td>
				<td><select id='bank' name='bank' onfocus='focusColor(this)' onblur='blurColor(this)'>
					<option value='not_selected'>---Select---</option>
					<option value='State Bank Of India'  <%=(laObj.getBankName().equals("State Bank Of India")?"selected":"") %> >State Bank of India</option>
					<option value='Punjab National Bank' <%= (laObj.getBankName().equals("Punjab National Bank")?"selected":"") %> >Punjab National Bank</option>
					<option value='Indian Bank' <%=(laObj.getBankName().equals("Indian Bank")?"selected":"") %> >Indian Bank</option>
					<option value='Syndicate Bank' <%=(laObj.getBankName().equals("Syndicate Bank")?"selected":"") %> >Syndicate Bank</option>
					<option value='Oriental Bank Of Commerce' <%=(laObj.getBankName().equals("Oriental Bank Of Commerce")?"selected":"") %> >Oriental Bank of Commerce</option>
					</select><label id='bank_label'></label>
				</td>
			</tr>		
			<tr>
				<td>Account No.</td>
				<td><input type='textbox' id='acc_no' name='acc_no' size='30' maxlength='14' value='<%=laObj.getAccountNumber()%>' onfocus='focusColor(this)' onblur='blurColor(this)'><label id='acc_no_label'><%= msg %></label></td>
			</tr>			
			<tr>
			...
			<input type="hidden" name="h_field" value="edt2"></input>
